...
func newWukong(programAddress, tokenAddress, tokenAccountAddress string) (wk wukong, err error) {
	...
	wk.ownerAccountAddress, err = wk.programAddress.FindProgramDerivedAddress([][]byte{[]byte("owner")})
	if err != nil {
...
func (wk wukong) ClaimWithRecommendInstruction(wallet Wallet.HostedWallet, referer Address.TokenAccountAddress, claimData wukongClaimData) (ix solana.Instruction, err error) {
	creditAddress, err := wk.programAddress.FindProgramDerivedAddress([][]byte{[]byte("credit"), []byte(claimData.SteamId)})
	if err != nil {
		err = gerror.Wrapf(err, "生成已购校验账户失败")
		return
	}

	recipientAddress, err := wk.programAddress.FindProgramDerivedAddress([][]byte{[]byte("recipient"), wallet.Account.Address.Bytes()})
	if err != nil {
	...
	ix, err = Instruction.Custom{
		...
		Accounts: []*solana.AccountMeta{
			...
			wallet.Account.Address.Meta().WRITE().SIGNER(), // user
	...
	for fork := uint32(0); fork < 20; fork++ {
		...
		go func() {
			...
			for i := fork*200 + 50; i < (fork+1)*200; i++ {
				...
				//	Creator: altWallet.Account.Address,
