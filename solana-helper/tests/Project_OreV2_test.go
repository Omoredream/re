...
func TestOreV2Balance(t *testing.T) {
	...
	proofAddress, err := OreV2Program.FindProgramDerivedAddress([][]byte{[]byte("proof"), wallet.Account.Address.Bytes()})
	if err != nil {
	...
	lastTransactions, err := officialPool.GetAddressTransactions(ctx, proofAddress, Utils.Pointer(10))
	if err != nil {
