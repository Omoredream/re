...
func TestEclipseMinesMoney(t *testing.T) {
	err := testEclipseMinesMoney(ctx)
	if err != nil {
		g.Log().Fatalf(ctx, "%+v", err)
	}
}

func testEclipseMinesMoney(ctx g.Ctx) (err error) {
	appWalletAddress := Address.NewFromBase58("DASLkpAhK4BVBnRt3zVDH6Rq8wQXg4fV3BJJ25a6agUM")

	turnKey, err := apikey.FromTurnkeyPrivateKey("a72a50ccd96f1f2a3f5c6a81a8dc43a6199a4afc8c8fbc757fa0647b49b0c38b", apikey.SchemeP256)
	if err != nil {
	...
	turnKeySign := func(data []byte, apiKey *apikey.Key, client *req.Client) (signature []byte, err error) {
		payload := gjson.MustEncode(g.MapStrAny{
			...
			"organizationId": "a55ab417-77e8-437b-a180-735011c06452",
	...
	client := req.C().
		SetCommonHeader("origin", "https://mines.money").
		SetCommonHeader("referer", "https://mines.money/").
	...
	for {
		...
		generateTxResp, err = client.R().
			...
			Post("https://mines.money/api/rpc")
		if err != nil {
		...
		submitResp, err = client.R().
			...
			Post("https://mines.money/api/bet")
		if err != nil {
