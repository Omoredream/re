...
func testIxReorganize(ctx g.Ctx) (err error) {
	...
	err = Instruction.BurnTokens{
		{
			...
			Token:        Utils.Must(officialPool.TokenCacheGet(ctx, Address.NewFromBase58("2PLc1Cbm7wJo8vKon54wGtxb1QcoyFnTut86BYpbdHKk").AsTokenAddress())),
	...
	err = Instruction.CloseTokenAccounts{
		{
			TokenAccount: wallet.Account.Tokens[Address.NewFromBase58("2PLc1Cbm7wJo8vKon54wGtxb1QcoyFnTut86BYpbdHKk").AsTokenAddress()].Address,
			Owner:        wallet.Account.Address,
...
func testIxCustom(ctx g.Ctx) (err error) {
	...
	raydiumCpmmAuthority, err := raydiumCpmmProgram.FindProgramDerivedAddress([][]byte{
	...
	err = Instruction.Custom{
		...
		Accounts: solana.AccountMetaSlice{
			...
			wallet.Account.Address.Meta().WRITE().SIGNER(),                                       // user_source_owner
			// Raydium AMM V4
			raydiumAmmV4Program.Meta(),
			consts.TokenProgramAddress.Meta(), // token_program
			Address.NewFromBase58("2FnyFeEwG1wXNBFKgc6RrN9YZk63hGKW5E1Mh8mXBiFo").Meta().WRITE(), // amm
			...
			wallet.Account.Address.Meta().WRITE().SIGNER(),                                       // payer
