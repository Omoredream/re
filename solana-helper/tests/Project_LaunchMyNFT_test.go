...
func testLaunchMyNFT(ctx g.Ctx) (err error) {
	...
	coreBankAddress, err := launchMyNFTProgram.FindProgramDerivedAddress([][]byte{
		[]byte("core_bank"),
	...
	for {
		for i, fleetWallet := range fleetWallets {
			...
			totalMints, err = launchMyNFTProgram.FindProgramDerivedAddress([][]byte{
				[]byte("TotalMints"),
			...
			err = Instruction.Custom{
				...
				Accounts: solana.AccountMetaSlice{
					fleetWallet.Account.Address.Meta().WRITE().SIGNER(),                                  // buyer
					...
					nftWallet.Account.Address.Meta().WRITE().SIGNER(), // asset
					// remaining
					consts.Token2022ProgramAddress.Meta(),
