...
import (
	...
	officialWebSocket "git.wkr.moe/web3/solana-helper/blockchain/solana/rpc/official/rpc/websocket"
)
...
func (pool *RPCs) websocketWaitForConfirmation(ctx g.Ctx, signature solana.Signature) (err error) {
	...
	err = pool.websocketAll(ctx, func(ctx g.Ctx, websocketRPC *officialWebSocket.RPC) (err error) {
		_, err = sendandconfirmtransaction.WaitForConfirmation(ctx, websocketRPC.Client(), signature, Utils.Pointer(1*time.Minute))
		return
	})
