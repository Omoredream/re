...
func (pool *RPCs) GetTokenHolders(ctx g.Ctx, token Token.Token) (tokenHolders []Account.Account, err error) {
	...
	owners, err = pool.getTokenAccountsOwner(ctx, Utils.Map(getTokenLargestAccountsResult.Value, func(value *rpc.TokenLargestAccountsResult) Address.TokenAccountAddress {
