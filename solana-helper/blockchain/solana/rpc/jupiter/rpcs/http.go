...
func (pool *RPCs) MultiStep(ctx g.Ctx, targetTokens []string, do func(ctx g.Ctx) (err error)) (err error) {
...
func (pool *RPCs) httpGetRandomSupportToken(ctx g.Ctx, excludes ...string) (out string, err error) {
	err = pool.http(ctx, excludes, func(ctx g.Ctx, httpRPC *jupiterHTTP.RPC) (err error) {
...
func (pool *RPCs) httpGetQuote(ctx g.Ctx, inputMint string, outputMint string, amount uint64) (out jupiterHTTP.GetQuoteResponse, err error) {
	err = pool.http(ctx, []string{inputMint, outputMint}, func(ctx g.Ctx, httpRPC *jupiterHTTP.RPC) (err error) {
		out, err = httpRPC.GetQuote(ctx, inputMint, outputMint, amount, jupiterHTTP.GetQuoteOptionalArgs{
			SlippageBps:      Utils.Pointer[uint16](0),
			OnlyDirectRoutes: Utils.Pointer(false),
		})
		return
	})
...
func (pool *RPCs) httpCreateSwapTransaction(ctx g.Ctx, userPublicKey string, quoteResponse jupiterHTTP.GetQuoteResponse) (out jupiterHTTP.CreateSwapTransactionResponse, err error) {
	...
	err = pool.http(ctx, targetTokens, func(ctx g.Ctx, httpRPC *jupiterHTTP.RPC) (err error) {
		out, err = httpRPC.CreateSwapTransaction(ctx, userPublicKey, quoteResponse, jupiterHTTP.CreateSwapTransactionOptionalArgs{
			WrapAndUnwrapSol:              Utils.Pointer(false),
			UseSharedAccounts:             Utils.Pointer(false),     // 套利时无法开启
			ComputeUnitPriceMicroLamports: Utils.Pointer[uint64](0), // 不使用优先费
			DynamicComputeUnitLimit:       Utils.Pointer(true),      // 动态 CU 限制
			SkipUserAccountsRpcCalls:      Utils.Pointer(false),
		})
		return
	})
