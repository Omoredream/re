...
type SetCULimit struct {
	Limit uint32
}

type SetCULimits []SetCULimit

func (tx SetCULimit) ToIx() (ix solana.Instruction, err error) {
...
func (tx SetCULimit) AppendIx(ixs *[]solana.Instruction) (err error) {
	ix, err := tx.ToIx()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
...
func (txs SetCULimits) ToIxs() (ixs []solana.Instruction, err error) {
...
func (txs SetCULimits) AppendIxs(ixs *[]solana.Instruction) (err error) {
	ixs_, err := txs.ToIxs()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
...
func IsSetCULimit(ix solana.Instruction) (ok bool, err error) {
...
func (tx *SetCULimit) Deserialize(ix solana.Instruction) (err error) {
	data, err := ix.Data()
	if err != nil {
		err = gerror.Wrapf(err, "无法读取指令数据")
		return
