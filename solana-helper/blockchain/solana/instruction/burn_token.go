...
type BurnTokens []BurnToken

func (tx BurnToken) ToIx() (ix solana.Instruction, err error) {
	ix, err = ProgramToken.NewBurnInstruction(
...
func (tx BurnToken) AppendIx(ixs *[]solana.Instruction) (err error) {
	ix, err := tx.ToIx()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
...
func (txs BurnTokens) ToIxs() (ixs []solana.Instruction, err error) {
...
func (txs BurnTokens) AppendIxs(ixs *[]solana.Instruction) (err error) {
	ixs_, err := txs.ToIxs()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
