...
var flashBorrowDiscriminator = consts.KaminoLendingProgramAddress.GetDiscriminator("global", "flash_borrow_reserve_liquidity")

type FlashBorrow struct {
	Token                    Token.Token
...
type FlashBorrows []FlashBorrow

func (tx FlashBorrow) ToIx() (ix solana.Instruction, err error) {
	ix, err = Instruction.Custom{
		...
		Accounts: []*solana.AccountMeta{
			...
			tx.MarketReserveLiquidity.Meta().WRITE(),   // reserveSourceLiquidity
			tx.UserTokenAccount.Meta().WRITE(),         // userDestinationLiquidity
		...
		Discriminator: flashBorrowDiscriminator,
		Data: Utils.Append(
...
func (tx FlashBorrow) AppendIx(ixs *[]solana.Instruction) (err error) {
	ix, err := tx.ToIx()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
...
func (txs FlashBorrows) ToIxs() (ixs []solana.Instruction, err error) {
...
func (txs FlashBorrows) AppendIxs(ixs *[]solana.Instruction) (err error) {
	ixs_, err := txs.ToIxs()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
