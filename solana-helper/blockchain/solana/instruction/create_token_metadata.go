...
type CreateTokenMetadatas []CreateTokenMetadata

func (tx CreateTokenMetadata) ToIx() (ix solana.Instruction, err error) {
...
func (tx CreateTokenMetadata) AppendIx(ixs *[]solana.Instruction) (err error) {
	ix, err := tx.ToIx()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
...
func (txs CreateTokenMetadatas) ToIxs() (ixs []solana.Instruction, err error) {
...
func (txs CreateTokenMetadatas) AppendIxs(ixs *[]solana.Instruction) (err error) {
	ixs_, err := txs.ToIxs()
	if err != nil {
		err = gerror.Wrapf(err, "将交易转换为基本指令失败")
		return
